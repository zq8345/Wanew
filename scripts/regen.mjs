...
//    (品类显示名的真源是 data/forms.json,applyFormNames() 会用它覆盖 chrome.json 里的 header.*。
//     所以这里写 header.power_charging 而线上渲染出 "Charging" —— 是对的,别改成写死字符串。)
//
// 🔴 【什么时候该用这一格,什么时候不该】—— 写在这里,免得它变成万能覆盖口子:
//    该用:某一页的页头【没有活真源】,或者它需要与同类页不同的页头,而差异是【结构性的、长期的】。
//         /products/ 属于前者:它的 h1 自一次性迁移后就没有任何函数在写。
//    不该用:
//      · 只是想改某个键的文案 —— 改 catalog,别在这里开个后门覆盖它;
//      · 想给一批页面加同一样东西 —— 那说明它属于 setListLabels 或模板,不是"按页覆盖";
//      · 临时对付某个页面不好看 —— 覆盖会把问题从"样式没统一"变成"样式在两个地方定义"。
//    ⚠️ 判据:如果第二个页面也要同样的覆盖,先停下想想那是不是该进公共路径。
//       **这一格是给例外用的;例外多到有规律时,规律就该上移。**
const LIST_PAGES = [
