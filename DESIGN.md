...
2. **对比节奏 + 选择性提亮**:长暗段插焦点,暗→亮→暗呼吸。
   - ⭐⭐ **提亮首选(Joe 2026-07-27 直裁,canonical)= 【深底保留 + 容器翻白】** `.w3-whitecards`:section 背景**留深色**、section header(eyebrow/h2/sub)**留浅字**,只把【卡片容器】翻白(白底 + 深字 `--w3-invert-ink`/`ink2` + 深蓝 `--w3-invert-accent` + 深底上的柔和落影)。白卡在深底上强对比跳出=治沉闷。
     🔴 **2026-08-01:落点归零。** Our Advantages 是它最后一个消费者,Joe 直接点名改成格子组(与 About 的 OEM/ODM 同一种容器语言)后,`.w3-whitecards` **全站 class 属性命中 = 0**。
     ⇒ 这条"提亮首选"目前**没有任何页面在用**。留着这段是因为它记的是 Joe 的裁定和理由,不是因为它还在生效;**要用它必须重新挂类,别以为它还开着**。相关的 `.w3-whitecards .w3-iconcard/.about-oemblock/.about-steps` 若干条同样失去消费者,归入死代码批次统一清。
     🔴 **2026-08-01 Joe 收回 Guides 那个落点**:首页曾同时挂两个提亮段且【紧挨着】,Joe「这两个板块的卡片跟其他的不太一样,contact 页面那种才是我们的主要风格」。本条自己就写着**「每屏 ≤1 个提亮段」**——两个都白 = 都不白,和 §卡片review 那句「当所有东西都被提出来,就等于没有东西被提出来」是同一条。**判据(定死,别再各凭手感):提亮只给【信任/证据类】板块**;Guides 是内容索引,不是证据,回深色浮起卡。
...
- **优势格**(`.w3-why` / `.w3-whycard`)〔2026-08-01 Joe 直接点名:与 About 的 OEM/ODM 同一种容器语言〕:**格子组**,不是卡片组 —— 四条优势是一个承诺被切成四份,不是四个各自独立、可点击的对象。写法照 §格子组三条判据:共用一个外框(`--w3-line` 1px + 14 圆角 + `overflow:hidden`)、`gap:1px` 露出 `--w3-line` 当发丝分隔、**格内零 hover**、格底与板块同色(**深底 `--w3-bg`**,靠发丝线而非色差分格,与 `.ab8-oem__grid` 的 bg2 同构)。标题 16px + B2B 实质正文(不截断,SEO 需要)。
  ⚠️ **两条已作废的旧描述,留着是为了不被重新捡回来**:① 旧版是「面板底+发丝边+16 圆角+浮起」的**独立白卡 + `translateY` hover** —— 那是卡片语言,不可点的东西不该给点击暗示;② 旧版规定「用编号索引 `.w3-whycard__idx`(01–04)领起,不用图标」—— 实测该类**产出页 0 命中、模板 0 命中,从来没被渲染过**,是一条只活在文档里的规定。**不用图标这一条仍然有效**(图标+三词卡=廉价模板感),但理由不再是"改用编号"。
...
- **提亮:深底白容器** `.w3-whitecards`〔⭐Joe 2026-07-27 直裁 · §2.6 招2 canonical〕:section 修饰类,**背景留深、header 留浅字**,只把内部卡片容器翻白——`.w3-whycard` → 白底 + 深字(`--w3-invert-ink`/`ink2`)+ 深蓝 accent(`--w3-invert-accent`)+ 深底落影。🔴 **2026-08-01 落点归零**(Our Advantages 改格子组,见 §3 优势格),`.w3-whitecards` 全站命中 0,这段现在记的是裁定不是现状 —— 详见 §2.6 招2 那条。
  ⚠️ **同日订正一条假描述**:原文写「`.w3-whycard` 被 About 复用(全暗做对),禁全局翻白」。实测 `.w3-whycard` **只有 home.html 一处、About 零命中** —— 那个"复用"从来不存在。写它的人是在防一个想象出来的消费者,而**假约束的代价是真的**:它让这个类看起来动不得。⇒ 写"某处复用了它"之前,先 grep 一次。
...
- **首页 Guides 卡**〔#82 → **2026-08-01 退役**〕:`.tj-gcard`/`.tj-cover`/`.tj-gbody`/`.tj-gmeta`/`.tj-cat`/`.tj-date`/`.tj-more`/`.tj-arw` 与 `.w3-guides` 网格**已整族删除**(20 条规则),孤儿键 `shared.jul_4_2026` 一并删。
  首页 Guides 板块改用 **`.guides-grid`/`.guides-card`**(= /guides/ 页同一套,眉标+标题+摘要+箭头)。两条理由:
  ① 全站主风格是深色浮起面板,白卡是提亮手段而非默认(见 §2.6 的收窄);
  ② Joe:「上面的图标其实跟内容不太搭,纯粹是为了放个图标而放图标」——`.tj-cover` 的底衬令牌 `--w3-well` 原注释就是「图标空 void 垫底(**补白**)」,它存在的理由是填空不是表意;三张卡的日期还是同一个批量值 `Jul 4, 2026`。**新卡没有图标位。**
  ⇒ 顺带与「首页板块标题=目标页 hero 标题」同源:首页 Guides 现在连**卡片**都和 /guides/ 一样。
