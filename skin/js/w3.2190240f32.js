...
/* Guides chip 条带:当前项进场即在可视区 + 右缘渐隐随滚动状态更新。
   ⚠️ 刻意【不用】 el.scrollIntoView():它会连祖先一起滚动,页面一加载就自己往下跳
      (总工点名的坑)。这里只写条带自己的 scrollLeft,不碰任何祖先。
   ⚠️ 遮罩必须随状态摘掉 —— 滚到底还挂着渐隐等于永远在骗人。 */
(function () {
  var nav = document.querySelector('.guides-nav');
  if (!nav) return;
  function sync() {
    var over = nav.scrollWidth - nav.clientWidth;
    nav.classList.toggle('is-nooverflow', over <= 1);
    nav.classList.toggle('is-end', over <= 1 || nav.scrollLeft >= over - 1);
  }
  var act = nav.querySelector('.guides-chip.is-active');
  if (act && nav.scrollWidth > nav.clientWidth) {
    // 把当前项摆进可视区:够不着就滚到它左缘前留一点余量,让人看得出左边还有东西
    var need = act.offsetLeft + act.offsetWidth - nav.clientWidth;
    if (act.offsetLeft < nav.scrollLeft) nav.scrollLeft = Math.max(0, act.offsetLeft - 16);
    else if (need > nav.scrollLeft) nav.scrollLeft = need + 16;
  }
  sync();
